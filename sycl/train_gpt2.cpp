...
dpct::device_info deviceProp; // fills in common_start()
...
typedef struct dpct_type_177263 {
...
typedef struct dpct_type_184086 {
...
typedef struct dpct_type_828196 {
    floatX* wte; // (V, C)
    floatX* wpe; // (maxT, C)
    floatX* ln1w; // (L, C)
    floatX* ln1b; // (L, C)
...
typedef struct dpct_type_471827 {
...
typedef struct dpct_type_700314 {
    ...
    sycl::int4 *bucket_info; // encoder_backward, B*T*num_c_groups (int4) - size
                             // for worst case
} GPT2;
...
void gpt2_backward(GPT2 *model, int* inputs, sycl::queue &q) {
    ...
    if (model->grads_memory == NULL) {
        ...
        model->bucket_info =
            (sycl::int4 *)mallocCheck(sizeof(sycl::int4) * model->batch_size *
            ...
            q.submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1) {
                        copy_and_cast_kernel(model_master_weights_ct0,
                                             params_memory_shard_offset_ct1,
                                             shard_num_parameters, item_ct1);
                    });
            });
        }
    ...
    for (int i = 0; i < NUM_PARAMETER_TENSORS; i++) {
        ...
        if(offset + num_parameters > shard_offset && offset < shard_offset + shard_num_parameters) {
            ...
            if(offset <= shard_offset) {
                ...
                local_params =
                    std::min(shard_num_parameters,
                             (offset + num_parameters) - shard_offset);
            } else {
                ...
                local_params =
                    std::min(num_parameters, shard_num_parameters - delta);
            }
            ...
            
            {
                ...
                q.parallel_for(
                    sycl::nd_range<3>(sycl::range<3>(1, 1, num_blocks) *
                                      ...
                                      sycl::range<3>(1, 1, block_size)),
                    [=](sycl::nd_item<3> item_ct1) {
                        adamw_kernel3(params_ptr, master_ptr, grad_ptr, m_ptr,
                                      v_ptr, local_params, learning_rate, beta1,
                                      beta2, beta1_correction, beta2_correction,
                                      eps, wd, grad_scale, seed, item_ct1);
                    });
            }
        }
...
float gpt2_estimate_mfu(GPT2 *model, int num_tokens, float dt) {
    ...
    */
    ...
    float flops_promised =
        get_flops_promised(deviceProp.get_name(), PRECISION_MODE) * 1e12f;
    if(flops_promised < 0) {
...
int main(int argc, char *argv[]) {
    ...
    printf0("| peak TFlops           | %-50.1f |\n",
            get_flops_promised(deviceProp.get_name(), PRECISION_MODE));
